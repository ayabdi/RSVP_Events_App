...
export const EventCard: FC<EventCardProps> = (props): JSX.Element => {
  {
    ...
    const attending =
      props.event.Invitees && countAttending(props.event.Invitees);
    const notAttending =
      props.event.Invitees && countNotAttending(props.event.Invitees);
    ...
    return (
      <div className="mt-12  mx-auto  md:max-w-none">
        <div className="relative w-full h-full group cursor-pointer">
          ...
          <div
            className="relative pb-80 rounded-xl overflow-hidden"
            onClick={() => router.push(`../event/${props.event.id}`)}
            style={{ backgroundColor: "rgb(241, 89, 74)" }}
          >
            <img
